...
- Added per-slot Conversation Call clip generation from Character Gallery empty/error clip cards, so individual idle/talking/reaction clips can be tested without generating the full batch.
- Added Video Generation services for unified Google AI Studio (Gemini Omni/Veo), OpenRouter, and Seedance 2.0, including connection defaults, asynchronous job polling, MP4 download handling, OpenRouter first-frame references, Veo first/last-frame avatar interpolation, and Seedance first/last-frame URL references for cleaner generated loops.
...
- Added Clips tabs to Character and Persona Galleries: characters show video-call presence clips, custom call clips, and generated scene/game videos tied to them, while personas show generated scene/game videos from chats using that persona.
- Added user-uploaded MP4 clips to Character and Persona Galleries: Character clips can replace standard video-call presence slots such as Idle or Talking or be stored as extra custom call clips, Persona clips can keep uploaded custom videos, and custom call-clip libraries now allow up to 128 entries.
...
- Fixed Character Gallery call-clip generation status so stale `generating` slots from older jobs no longer light up when only one clip, such as Idle, is being generated; Google Veo video extraction now also accepts more completed-operation shapes and reports provider no-video reasons.
...
- Fixed Character Gallery call-clip prompts to use the provider-effective clip duration, so Google Veo avatar-reference clips now ask for the same 8-second loop length that Veo actually generates.
...
- Clarified Seedance 2.0 local avatar reference handling: official Seedance image-to-video requires public HTTPS first/last-frame URLs, so Marinara now fails early with setup guidance when `VIDEO_REFERENCE_PUBLIC_BASE_URL` is missing instead of sending unsupported local or data-URI media references.
...
- Fixed Conversation Call video prompts so generated standard/custom character clips receive relevant character-card description, appearance, personality, and scenario notes, helping video providers preserve visual constraints such as masks or hidden eyes.
