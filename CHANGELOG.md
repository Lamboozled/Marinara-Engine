...
- Added 8-Ball Pool as a one-on-one Conversation-mode table game: a real 2D table with seeded break scatter and deterministic shot resolution, an engine-computed shot menu (direct pots, bank shots, and safeties, each with an honest difficulty tier and make percentage) that characters choose from in personality — daredevils take the showoff bank, tacticians play safe — plus controlled/aggressive shot styles, scratches with ball-in-hand, called 8-ball pockets, alternate breaks, and race-to-N matches with player-paced racks.
...
- 8-ball joins the `[eightball]` Conversation command family, with a `/8ball` slash command (alias `/pool`), natural-language launcher, per-chat command toggle, and a setup modal for the opponent, announcer, match length, and who breaks; the board renders the table, balls, aim lines, and the same shot menu the characters see.
