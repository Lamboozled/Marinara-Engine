...
export const SPRITES_ANIMATED_PORTRAIT: PromptOverrideKeyDef<SpritesAnimatedPortraitCtx> = {
  ...
  defaultBuilder: (ctx) =>
    [
      ...
      `Start on the neutral reference face, transition into ${ctx.expression}, hold the expression briefly, then return to the neutral reference face by the final frame so the GIF loops cleanly.`,
